...
const VALIDATOR = Validation.object().shape({
  [FIELDS.firstName]: Validation.string().required(),
  [FIELDS.lastName]: Validation.string().required(),
  [FIELDS.birthdate]: Validation.date().required(),
  [FIELDS.latestCsuiClassYear]: Validation.date().required(),
  [FIELDS.latestCsuiProgram]: Validation.string().required(),
  [FIELDS.uiSsoNpm]: Validation.number().min(16).max(16),
  [FIELDS.email]: Validation.string().email().required(),
  [FIELDS.password]: Validation.string(),
  [FIELDS.repassword]: Validation.string()
});
        ...
        <Grid item xs={6}>
          <MuiPickersUtilsProvider utils={MomentUtils}>
            <InlineDatePicker
              ...
              onChange={date => setFieldValue(FIELDS.latestCsuiClassYear, date)}
              margin="normal"
              variant="outlined"
              format="YYYY"
              views={["year"]}
